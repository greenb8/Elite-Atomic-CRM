...
const ProductCard = () => {
	...
	return (
		<Card sx={{ width: 220 }}>
			<CardActionArea onClick={() => redirect('show', 'products', record.id)}>
				{firstPhoto ? (
					<CardMedia component="img" height="140" image={firstPhoto} alt={record.name} />
				) : (
				...
				<CardContent>
					...
					{record.price != null ? (
						<Typography variant="body2" color="text.secondary">
							{Number(record.price).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
						</Typography>
					) : null}
				</CardContent>
			</CardActionArea>
